...
export function useCivitaiModelApi2(
    modelId: Ref<number> = ref(-1),
    _modelVersionId: Ref<number> = ref(-1),
    _selectedVersionIndex: Ref<number> = ref(-1),
    selectedModelVersionPreviewImageIndex: Ref<number> = ref(-1),
    ...
    const modelVersionId = computed({
        ...
        set: (value) => {
            const versionIndex = getVersionIndexFromId(value)
            const _versionCount = versionCount.value

            _selectedVersionIndex.value = Math.max(0, Math.min(versionIndex, _versionCount))
    ...
    const selectedVersionIndex = computed({
        ...
        set: (value) => {
            ...
            modelVersionId.value = getVersionIdFromIndex(_value)
        }
    })

    const modelApiUrl = computed(() => {
        const _mid = modelId.value
